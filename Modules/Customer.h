                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        switch(choice)
                        {
                            ...
                            case 5:
                                bzero(writeBuffer, sizeof(writeBuffer));
                                strcpy(writeBuffer, "Enter destination account number: ");
                                write(connectionFD, writeBuffer, sizeof(writeBuffer));
    ...
    else
    {
        ...
        else
        {
            ...
            if (customer.balance < withdrawAmount){
                ...
                printf("Insufficient balance in the account: %d\n", accountNumber);

                sprintf(writeBuffer, "Insufficient funds in account!^");
                writeBytes = write(connectionFD, writeBuffer, sizeof(writeBuffer));
                read(connectionFD, readBuffer, sizeof(readBuffer));
                if(writeBytes == -1)
                {
...
void applyLoan(int connectionFD, int accountNumber){
    ...
    printf("Applied loan of %d from account no.: %d\n", amount, accountNumber);
...
void transferFunds(int connectionFD, int sourceAccount, int destAccount, float amount) {
    ...
    if(dstOffset == -1)
    {
        ...
        strcpy(writeBuffer, "Account dosen't Exist!^");
        write(connectionFD, writeBuffer, sizeof(writeBuffer));
        read(connectionFD, readBuffer, sizeof(readBuffer));
        return;        
    }
...
void addFeedback(int connectionFD){
    ...
    if(choice == 1)
    {
        strcpy(fb.feedback, "Best Service");
    }
    else if(choice == 2)
    {
        strcpy(fb.feedback, "Good service");
    }
    else
    {
        strcpy(fb.feedback, "Bad service");
    }
