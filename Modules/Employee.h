...
void approveRejectLoan(int connectionFD, int empID)
{
    ...
    if(result1 != -1)
    {
        ...
        int choice;
        bzero(writeBuffer, sizeof(writeBuffer));
        strcpy(writeBuffer, "Enter 1 to Approve the Loan\nEnter 2 to Reject the Loan: ");
        write(connectionFD, writeBuffer, sizeof(writeBuffer));
    ...
    {
        strcat(writeBuffer, "Account is already deactivate so we can't approve or reject loan\n");
    }
    strcat(writeBuffer, "^");
    write(connectionFD, writeBuffer, sizeof(writeBuffer));
    read(connectionFD, readBuffer, sizeof(readBuffer));
