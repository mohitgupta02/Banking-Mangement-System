        ...
        switch (choice) {
            case 1:
                ...
                if(addEmployee(connectionFD))
                {
                    bzero(writeBuffer, sizeof(writeBuffer));
                    bzero(readBuffer, sizeof(readBuffer));
                    strcpy(writeBuffer, "Employee added successfully\n^");
                    write(connectionFD, writeBuffer, sizeof(writeBuffer));
                    read(connectionFD, readBuffer, sizeof(readBuffer));
                }
                break;
            ...
            default:
                bzero(writeBuffer, sizeof(writeBuffer));
                bzero(readBuffer, sizeof(readBuffer));
                strcpy(writeBuffer, "Invalid! Please try again.^");
                write(connectionFD, writeBuffer, sizeof(writeBuffer));
                read(connectionFD, readBuffer, sizeof(readBuffer));
        }
    }    
...
void manageRole(int connectionFD)
{
    ...
    while(read(file, &emp, sizeof(emp)) != 0)
    {
        if(emp.empID == id)
        {
            int choice;
            bzero(writeBuffer, sizeof(writeBuffer));
            strcpy(writeBuffer, "Enter 1 to make user manager\nEnter 2 to make user employee: ");
            write(connectionFD, writeBuffer, sizeof(writeBuffer));
